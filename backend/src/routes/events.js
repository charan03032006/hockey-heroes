...
router.post('/matches/:matchId/events', async (req, res) => {
  const {
    player_id, team_id, type, period, game_time, x, y, penalty_minutes,
    outcome, related_player_id, card_type, substitution_in, substitution_out,
  } = req.body;
  ...
  if (!Number.isInteger(Number(period)) || Number(period) < 1 || Number(period) > 4) {
    return res.status(400).json({ error: 'period must be 1, 2, 3 or 4.' });
  }
  ...
  if (type === 'substitution' && (!substitution_in || !substitution_out)) {
    return res.status(400).json({ error: 'A substitution needs both an incoming and outgoing player.' });
  }

  const { data: match, error: matchError } = await supabase
    .from('match')
    .select('id,home_team_id,away_team_id,status,current_period,clock_seconds')
  ...
  const payload = {
    match_id: req.params.matchId, player_id: player_id || null, team_id, type,
    period: Number(period), game_time: game_time || `${Math.floor((match.clock_seconds || 0) / 60)}:${String((match.clock_seconds || 0) % 60).padStart(2, '0')}`,
    x: x ?? null, y: y ?? null, penalty_minutes: penalty_minutes ?? null,
    outcome: outcome ?? null, related_player_id: related_player_id || null,
    card_type: card_type || null, substitution_in: substitution_in ?? null, substitution_out: substitution_out ?? null,
  };
